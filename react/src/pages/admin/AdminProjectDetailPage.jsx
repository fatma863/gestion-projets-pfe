...
import {
  ...
  BarChart3, Users, Calendar, Activity, Gauge, MessageSquare,
  Settings, Trash2, UserCheck,
} from 'lucide-react';
...
export default function AdminProjectDetailPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header admin avec contrôle total */}
        ...
        {[
          ...
          return (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`relative flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium transition-colors ${tab === t.key ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`}
            >
              <TabIcon size={15} />{t.label}
              {tab === t.key && <motion.div layoutId="admin-tab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary rounded-full" />}
            </button>
          );
        })}
      </div>
      ...
      {tab === 'kanban' && (
        <KanbanBoard statuses={statuses || []} tasksByStatus={tasksByStatus} onDrop={handleDrop}
      ...
      <TaskModal
        open={taskModal.open} onClose={() => setTaskModal({ open: false, task: null, statusId: null })}
        projectId={parseInt(id)} task={taskModal.task} defaultStatusId={taskModal.statusId} statuses={statuses || []}
      />
    </div>
  );
}
  ...
  return (
    <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar">
      {statuses.map((status, i) => {
        ...
        return (
          <motion.div key={status.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
            className="flex w-72 flex-shrink-0 flex-col rounded-xl bg-muted/30 border border-border/50"
          ...
          >
            <div className="flex items-center justify-between px-3 py-2.5">
              ...
              <button onClick={() => onAddTask(status.id)} className="rounded-md p-1 hover:bg-accent transition-colors">
...
function KanbanCard({ task, onClick }) {
  ...
  return (
    <div draggable onDragStart={handleDragStart} onDragEnd={handleDragEnd} onClick={onClick}
      className="group cursor-pointer rounded-xl border border-border/60 bg-white p-3 shadow-sm transition-all duration-200 hover:shadow-md hover:border-border active:scale-[0.98]"
    >
...
function MembersTab({ projectId }) {
  ...
  const { data: allUsers } = useQuery({ queryKey: ['search-users'], queryFn: () => api.get('/projects/search-users').then((r) => r.data.users) });

  const availableUsers = (allUsers || []).filter((u) => !members?.some((m) => (m.id || m.user_id) === u.id));
  const addMutation = useMutation({ mutationFn: (data) => api.post(`/projects/${projectId}/members`, data), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['project-members', projectId] }); setShowAdd(false); setUserId(''); } });
  const removeMutation = useMutation({ mutationFn: (uid) => api.delete(`/projects/${projectId}/members/${uid}`), onSuccess: () => queryClient.invalidateQueries({ queryKey: ['project-members', projectId] }) });
...
function ActivityTab({ projectId }) {
  ...
  if (isLoading) return <div className="flex justify-center py-10"><Spinner /></div>;
  if (!activities || activities.length === 0) return <Card><CardContent className="py-10 text-center text-muted-foreground">Aucune activité</CardContent></Card>;
  return (
    <div className="space-y-2">
      {activities.map((a) => (
...
function WorkloadTab({ projectId }) {
  const { data, isLoading } = useQuery({ queryKey: ['project-workload', projectId], queryFn: () => api.get(`/projects/${projectId}/dashboard`).then((r) => r.data) });
  if (isLoading) return <div className="flex justify-center py-10"><Spinner /></div>;
  return (
    <Card><CardContent className="p-6">
      <h3 className="font-semibold text-foreground mb-4">Charge de travail</h3>
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-lg bg-accent/30 p-4"><p className="text-sm text-muted-foreground">Heures estimées</p><p className="text-lg font-bold">{data?.total_estimated_hours || 0}h</p></div>
        <div className="rounded-lg bg-accent/30 p-4"><p className="text-sm text-muted-foreground">Heures réelles</p><p className="text-lg font-bold">{data?.total_actual_hours || 0}h</p></div>
        <div className="rounded-lg bg-accent/30 p-4"><p className="text-sm text-muted-foreground">Progression</p><p className="text-lg font-bold">{data?.avg_progress || 0}%</p></div>
      </div>
    </CardContent></Card>
  );
}
