...
export default function ManagerTeamDetailPage() {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['team', id] }); setShowAdd(false); setUserId(''); },
  });
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" onClick={() => setShowAdd(!showAdd)}><Plus className="mr-1 h-4 w-4" /> Ajouter un membre</Button>
        </div>

        {showAdd && (
          <Card><CardContent className="flex flex-wrap items-end gap-3 p-4">
            <div><label className="mb-1 block text-xs font-medium">ID utilisateur</label><Input type="number" value={userId} onChange={(e) => setUserId(e.target.value)} className="w-24" placeholder="ID" /></div>
            <div><label className="mb-1 block text-xs font-medium">Rôle</label><Select value={role} onChange={(e) => setRole(e.target.value)} className="w-36">{ROLES.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}</Select></div>
            <div><label className="mb-1 block text-xs font-medium">Capacité (h/sem)</label><Input type="number" min={0} max={60} value={capacity} onChange={(e) => setCapacity(e.target.value)} className="w-24" /></div>
            <Button size="sm" onClick={() => addMutation.mutate({ user_id: parseInt(userId), role_in_team: role, capacity_hours_per_week: parseInt(capacity) })} disabled={!userId || addMutation.isPending}>Ajouter</Button>
          </CardContent></Card>
        )}

        <div className="space-y-2">
          {team.members?.length === 0 && <Card><CardContent className="py-10 text-center"><Users className="mx-auto mb-3 h-10 w-10 text-muted-foreground" /><p className="text-sm text-muted-foreground">Aucun membre</p></CardContent></Card>}
          {team.members?.map((m) => (
            <div key={m.id} className="flex items-center justify-between rounded-lg border border-border bg-card p-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => { if (window.confirm('Retirer ce membre ?')) removeMutation.mutate(m.id); }} className="rounded p-1 text-muted-foreground hover:bg-red-50 hover:text-destructive"><Trash2 className="h-4 w-4" /></button>
              </div>
            </div>
          ))}
        </div>
